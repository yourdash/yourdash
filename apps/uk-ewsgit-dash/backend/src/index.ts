...
export default class Application extends YourDashApplication {
  constructor() {
    super({
            version: {
              major: 1, minor: 0,
            }, configVersion: 1, credits: {
        authors: [ { name: "Ewsgit", site: "https://ewsgit.uk" } ],
      }, frontend: true, displayName: "Dash", description: "The YourDash dashboard application.", id: "uk-ewsgit-dash",
          });
  ...
  public onLoad(): this {
    instance.request.get("/uk-ewsgit-dash/dashboard", {
      schema: {
        response: {
          200: z.object({
                          header: z.object({
                                             ...
                                             background: z.object({
                                                                    blur: z.number(), opacity: z.number(),
                                                                  }),
                                             font: z.object({
      ...
      const userData = await instance.database.query(
          "SELECT forename, surname FROM public.users WHERE username = $1", [ username ]);
      const dashboardData = await instance.database.query(
          "SELECT * FROM public.uk_ewsgit_dash_dashboard WHERE username = $1", [ username ]);

      const data = dashboardData.rows[0];

      return {
        header: {
          welcomeMessage: data.header_welcome_message, size: data.header_size, style: data.header_style, background: {
            blur: data.header_background_blur, // percentage from 0% to 100%
            opacity: data.header_background_opacity,
          }, font: {
            family: data.header_font_family || undefined, size: data.header_font_size, weight: data.header_font_weight,
          },
        }, content: {
          background: {
          ...
          type: data.background_type, value: data.background_type !== "image" ? data.background_value : undefined,
        }, user: {
          username: username, forename: userData.rows[0].forename, surname: userData.rows[0].surname,
        },
      };
    },);

    instance.request.post("/uk-ewsgit-dash/dashboard", {
      schema: {
        body: z.object({
                         header: z.object({
                                            welcomeMessage: z.string(),
      ...
      const body = req.body as {
        header: {
          welcomeMessage: string; size: "small" | "medium" | "large"; style: "floating" | "docked"; background: {
            blur: number; opacity: number;
          }; font: {
            family: string; weight: string; size: number;
          };
        }; background: | {
          type: "image";
        } | {
          ...
          type: "linearGradient"; value: string;
        } | {
          type: "radialGradient"; value: string;
        }; content: {
          background: {
            blur: number; opacity: number;
          }; pages: {
            id: string; data: any; dimensions: {
      ...
      instance.database.query(`UPDATE public.uk_ewsgit_dash_dashboard
    ...
    instance.request.get("/uk-ewsgit-dash/backgroundImage", async (req, res) => {
      return instance.requestManager.sendFile(
          res, path.join(instance.filesystem.commonPaths.SystemDirectory(), "loginBackground.avif"), "image/avif",);
    });
