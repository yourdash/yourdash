  ...
  public async onLoad(): Promise<this> {
                ...
                data: {
                  ...
                  capabilities: {
                    ...
                    theming: {
                      ...
                      url:
                        query.external_url ||
                        `${req.protocol}://${req.hostname || "localhost:3563"}`,
                      slogan:
                        query.description ||
                        "[YourDash Instance Description (Nextcloud slogan)]",
                      color: "#00679e",
                      "color-text": "#ffffff",
    ...
    this.instance.request.post(
      "/uk-ewsgit-nextcloud/index.php/login/v2",
      {
        schema: {
          response: {
            200: z.object({
              poll: z.object({ token: z.string(), endpoint: z.string() }),
              login: z.string(),
            }),
          },
        },
    ...
    this.instance.request.post(
      ...
      async (req, res) => {
        ...
        if (!authSession.sessionToken) {
          console.log(
            "polled and session was found but was missing a username?",
          );
          return res.status(404).send();
        }
        ...
        if (!authSession.sessionToken) {
          console.log(
            "polled and session was found but was not yet authenticated.",
          );
          return res.status(404).send();
        }
    ...
    async function getUserForRequest(
      ...
      if (!req.headers.authorization) {
        console.log(
          "We should not get here!!! getUserForRequest() // nextcloud",
        );
        // @ts-ignore
        return;
      }
      ...
      function parseAuthorization(sessionToken: string): {
        username: string;
        sessionToken: string;
      } {
        const tokenString = sessionToken.split("Basic ")[1];
        const tokenParsed = Buffer.from(tokenString, "base64").toString(
          "utf-8",
        );
