...
`;
      ...
      for (const [index, application] of this.applications.loadedApplications.entries()) {
        ...
        loadableRegionReplacement += `const Application${index}=lazy(()=>import("@yourdash/applications/${path.posix.join(path.basename(application.__internal_initializedPath), "./web/index.tsx")}"));`;
