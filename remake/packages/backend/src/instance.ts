...
class Instance {
    ...
    async __internal_init() {
        ...
        async function prepareDatabase() {
            ...
            try {
                let tempDatabaseClient = new bun.SQL({
                                                         password: self.configurationManager.config.postgresPassword,
                                                         username: self.configurationManager.config.postgresUser,
                                                         database: "postgres",
                ...
                try {
                    await tempDatabaseClient`CREATE DATABASE ${bun.sql(self.configurationManager.config.postgresDatabase)};`;
                } catch (e) {
                }
            } catch (e) {
                self.log.error(
                    "database",
                    `Failed to setup pre-startup connection to PostgreSQL Database, please ensure that you have PostgreSQL installed, and the default 'postgres' database exists. (${self.log.addEmphasisToString(
                        "Will retry in 10s")})`
                );
        ...
        try {
            this.log.info(
                "database",
                `Using ${this.log.addEmphasisToString(
                    `'${this.configurationManager.config.postgresDatabase}'`)} database`
            );
            this.database = new bun.SQL({
                                            ...
                                            username: this.configurationManager.config.postgresUser,
        ...
        try {
            await this.database`CREATE TABLE IF NOT EXISTS public.users
                                       (
        ...
        try {
            await this.database`CREATE TABLE IF NOT EXISTS teams
                                       (
        ...
        try {
            const doesConfigurationExist = await this.database`SELECT EXISTS (SELECT *
                                                                                     FROM pg_tables
                                                                                     WHERE schemaname = 'public'
                                                                                       AND tablename = 'configuration');`

            if (!doesConfigurationExist[0].exists) {
                this.log.info(
                    "database",
                    `Table ${this.log.addEmphasisToString("configuration")} will be created.`
                );
                await this.database`CREATE TABLE IF NOT EXISTS configuration
                                           (
                ...
                await this.database`
                    INSERT INTO public.configuration(creation_date) VALUES (${Date.now()});`
            }
        } catch (e) {
            console.error(e);
            this.log.error(
        ...
        try {
            await this.database`CREATE TABLE IF NOT EXISTS public.panel_configuration
                                       (
                                           config_version      serial primary key,
        ...
        const users = await this.database`SELECT username FROM public.users`
